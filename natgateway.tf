...
resource "aws_eip" "DaXPO-eip-for-nat-gateway-2" {
  vpc    = true
...
resource "aws_nat_gateway" "DaXPO-nat-gateway-1" {
  allocation_id = aws_eip.DaXPO-eip-for-nat-gateway-1.id    
  subnet_id     = aws_subnet.public-subnet-1.id

  tags   = {
    Name = "DaXPO NAT Gateway Public Subnet 1"
  }
}

# Create Nat Gateway 2 in Public Subnet 2
resource "aws_nat_gateway" "nat-gateway-2" {
  allocation_id = aws_eip.DaXPO-eip-for-nat-gateway-2.id
  subnet_id     = aws_subnet.public-subnet-2.id

  tags   = {
    Name = "DaXPO NAT Gateway Public Subnet 2"
...
resource "aws_route_table" "private-route-table-1" {
  ...
  route {
    ...
    nat_gateway_id  = aws_nat_gateway.DaXPO-nat-gateway-1.id
  }

  tags   = {
    Name = "DaXPO Private Route Table 1" 
  }
}
...
resource "aws_route_table" "private-route-table-2" {
  ...
  tags   = {
    Name = "DaXPO Private Route Table 2"
  }
}
